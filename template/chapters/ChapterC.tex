...
\subsubsection{随机更新策略}
  拉格朗日松弛法将原问题分解成了一个个独立的最短路径问题，这样使得算法可以并行化进行设计，但是由于个个子问题独立分离，使得每个问题再求最短路径时都是贪心的，这样可能会使得大量业务抢占同一批链路，造成拥塞，一旦拥塞，链路的权重增加，又会使得大量的业务放弃这一批链路，去抢占其他链路，这样恶性循环，使得算法难以收敛，比如图（）中的例子。
\begin{figure}
\setlength{\belowcaptionskip}{-0.1cm}
  \begin{center}
